...
class ReactActivityResultCallerThreadingTest {
  ...
  private class ThrowingLauncher : ActivityResultLauncherCompat<String>(GetContent()) {
    ...
    override fun unregister(): Unit = Unit
  }
  ...
  @Test
  fun `an exception from a posted launch does not escape the UI thread`() {
    val delegate = ThrowingLauncher()
    val failures = mutableListOf<RuntimeException>()
    val launcher = DeferredActivityResultLauncher("test", GetContent(), {}, failures::add)
    launcher.bind(registry, delegate)

    onBackgroundThread { launcher.launch("image/*") }
    drainMainLooper()

    assertThat(delegate.launchCount).isEqualTo(1)
    assertThat(failures).hasSize(1)
  }

  @Test
  fun `an exception from a queued launch does not escape bind`() {
    val delegate = ThrowingLauncher()
    val failures = mutableListOf<RuntimeException>()
    val launcher = DeferredActivityResultLauncher("test", GetContent(), {}, failures::add)
  ...
  @Test
  fun `the explicit failure handler receives launch exceptions`() {
    val failure = ActivityNotFoundException("No matching Activity")
    registry.launchFailure = failure
    val failures = mutableListOf<RuntimeException>()
    val launcher = caller.registerForActivityResult(moduleA, GetContent(), {}, failures::add)
    drainMainLooper()
  ...
  @Test
  fun `resuming an activity without a registry unbinds without throwing`() {
    val launcher = caller.registerForActivityResult(moduleA, GetContent()) {}
    drainMainLooper()
    val registryA = registry

    val plainActivity = Robolectric.buildActivity(Activity::class.java).create().get()
    whenever(reactContext.currentActivity).thenReturn(plainActivity)
    caller.onHostResume()
    drainMainLooper()

    assertThat(registryA.registeredKeys).isEmpty()
    launcher.launch("image/*")
    drainMainLooper()
    assertThat(registryA.launchThreads).isEmpty()

    val registryB = resumeNewActivity()
    caller.onHostResume()
    drainMainLooper()

    assertThat(registryB.registeredKeys).containsExactly(expectedKey)
    assertThat(registryB.launchThreads).hasSize(1)
  }
