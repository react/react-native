...
internal class ReactActivityResultCallerImpl(private val reactContext: ReactContext) :
  ...
  private fun <I, O> register(
    ...
    val launcher =
        DeferredActivityResultLauncher(
            key = key,
            contract = contract,
            onUnregister = { entries.remove(key) },
            onLaunchFailure = { exception -> onLaunchFailure.onActivityResult(exception) },
        )
  ...
  private fun bindSafely(entry: Entry<*, *>, registry: ActivityResultRegistry) {
    try {
      entry.bindTo(registry)
    } catch (exception: RuntimeException) {
      FLog.e(
          ReactConstants.TAG,
          "Failed to bind ActivityResult launcher '${entry.key}'.",
          exception,
      )
    }
  }

  private fun unbindSafely(entry: Entry<*, *>) {
    try {
      entry.launcher.unbind()
    } catch (exception: RuntimeException) {
      FLog.e(
          ReactConstants.TAG,
          "Failed to unbind ActivityResult launcher '${entry.key}'.",
          exception,
      )
    }
  }
