...
internal class SafeAreaInsetsObserver private constructor(private val view: View) :
  ...
  private fun maybeEmit() {
    // Only a change of the insets triggers an event: a view that moves
    // (scrolling, layout) without its overlap with the system UI changing stays
    // silent. This is what makes observing views safe to place inside scroll
    // views — and it prevents feedback loops, since the synchronous render
    // caused by an event produces a new frame, which runs this pre-draw
    // listener again.
