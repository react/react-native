...
public abstract class ReactContext extends ContextWrapper {
   ...
   * ActivityResultCallback)}, but reports a failure to start the deferred UI-thread launch through
   * {@code onLaunchFailure}.
   */
  public <I, O> ActivityResultLauncher<I> registerForActivityResult(
      Object owner,
      ActivityResultContract<I, O> contract,
      ActivityResultCallback<O> callback,
      ActivityResultCallback<RuntimeException> onLaunchFailure) {
    return getActivityResultCaller()
        .registerForActivityResult(owner, contract, callback, onLaunchFailure);
  }
  ...
  /**
   * Same as {@link #registerForActivityResult(Object, String, ActivityResultContract,
   * ActivityResultCallback)}, but reports a failure to start the deferred UI-thread launch through
   * {@code onLaunchFailure}.
   */
  public <I, O> ActivityResultLauncher<I> registerForActivityResult(
      ...
      ActivityResultCallback<RuntimeException> onLaunchFailure) {
    return getActivityResultCaller()
        .registerForActivityResult(owner, key, contract, callback, onLaunchFailure);
  }
