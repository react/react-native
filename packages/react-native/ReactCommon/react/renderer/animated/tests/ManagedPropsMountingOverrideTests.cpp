...
class ManagedPropsMountingOverrideTests : public AnimationTestsBase {
 protected:
  ...
  // value -> style -> props graph for an animated opacity; view left unconnected.
  OpacityGraph buildOpacityGraph(float initialOpacity) {
    ...
    return OpacityGraph{opacityTag, styleTag, propsTag, viewTag};
  }
...
TEST_F(
  ...
  nodesManager_->connectAnimatedNodeToView(g.propsTag, g.viewTag);
  // No runAnimationFrame: the value must be live the instant the view is managed.
