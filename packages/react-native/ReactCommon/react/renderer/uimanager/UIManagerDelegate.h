...
class UIManagerDelegate {
 public:
   ...
   */
  virtual void uiManagerDidPromoteReactRevision(
      const ShadowTree& shadowTree) = 0;

  using OnSurfaceStartCallback =
      std::function<void(const ShadowTree& shadowTree)>;
  virtual void uiManagerShouldAddOnSurfaceStartCallback(
      OnSurfaceStartCallback&& callback) = 0;

  // View transition bitmap snapshot capture and application.
  virtual void uiManagerDidCaptureViewSnapshot(
      Tag tag,
      SurfaceId surfaceId) = 0;
  virtual void uiManagerDidSetViewSnapshot(
      Tag sourceTag,
      Tag targetTag,
      SurfaceId surfaceId) = 0;
